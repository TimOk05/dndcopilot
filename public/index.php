        ...
        <div class="character-card">
                ...
                <div class="stat-group">
                    ...
                    <div class="stats-grid">
                        <div class="stat-item">
                            ...
                            <span class="stat-value">${character.abilities.str} (${character.modifiers.str >= 0 ? '+' : ''}${character.modifiers.str})</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-name">Ловкость</span>
                            <span class="stat-value">${character.abilities.dex} (${character.modifiers.dex >= 0 ? '+' : ''}${character.modifiers.dex})</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-name">Телосложение</span>
                            <span class="stat-value">${character.abilities.con} (${character.modifiers.con >= 0 ? '+' : ''}${character.modifiers.con})</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-name">Интеллект</span>
                            <span class="stat-value">${character.abilities.int} (${character.modifiers.int >= 0 ? '+' : ''}${character.modifiers.int})</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-name">Мудрость</span>
                            <span class="stat-value">${character.abilities.wis} (${character.modifiers.wis >= 0 ? '+' : ''}${character.modifiers.wis})</span>
                        </div>
                        <div class="stat-item">
                ...
                <div class="stat-group">
                    ...
                    <div class="combat-stats">
                        <div class="combat-stat">
                            <span class="combat-label">Хиты:</span>
                            <span class="combat-value">${character.hit_points}</span>
                        </div>
                        <div class="combat-stat">
                            <span class="combat-label">Класс брони:</span>
                            <span class="combat-value">${character.armor_class}</span>
                        </div>
                        <div class="combat-stat">
                            <span class="combat-label">Скорость:</span>
                            <span class="combat-value">${character.speed} футов</span>
                        </div>
                        <div class="combat-stat">
                        ...
                        <div class="combat-stat">
                            <span class="combat-label">Бонус мастерства:</span>
                            <span class="combat-value">+${character.proficiency_bonus}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            ${character.background ? `
                <div class="character-background">
                    <h4>Предыстория</h4>
                    <p><strong>${character.background.name}:</strong> ${character.background.description}</p>
                </div>
            ` : ''}
            ...
            ${character.equipment && character.equipment.items && character.equipment.items.length > 0 ? `
                <div class="character-equipment">
                    <h4>Снаряжение</h4>
                    <ul>
                        ${character.equipment.items.map(item => `<li>${item}</li>`).join('')}
                    </ul>
                    ${character.equipment.money ? `<p><strong>Деньги:</strong> ${character.equipment.money}</p>` : ''}
                </div>
            ` : ''}
            
            ${character.spells && (character.spells.cantrips.length > 0 || character.spells.level_1.length > 0) ? `
                <div class="character-spells">
                    ...
                    ${character.spells.cantrips.length > 0 ? `
                        <div class="spell-level">
                            <h5>Заговоры</h5>
                            <ul>
                                ${character.spells.cantrips.map(spell => `<li>${spell}</li>`).join('')}
                            </ul>
                        </div>
                    ` : ''}
                    ${character.spells.level_1.length > 0 ? `
                        <div class="spell-level">
                            <h5>1 уровень</h5>
                            <ul>
                                ${character.spells.level_1.map(spell => `<li>${spell}</li>`).join('')}
                            </ul>
                        </div>
                    ` : ''}
                </div>
            ...
            ${character.potions && character.potions.length > 0 ? `
                <div class="character-potions">
                    <h4>Зелья</h4>
                    <ul>
                        ${character.potions.map(potion => `<li>${potion.name} - ${potion.effect}</li>`).join('')}
                    </ul>
                </div>
            ` : ''}
...
function saveNewCharacterToNotes(character) {
    const noteContent = `
        <div class="character-note">
            ...
            <div class="character-note-stats">
                <div><strong>Хиты:</strong> ${character.hit_points}</div>
                <div><strong>Класс брони:</strong> ${character.armor_class}</div>
                <div><strong>Скорость:</strong> ${character.speed} футов</div>
                <div><strong>Инициатива:</strong> ${character.initiative >= 0 ? '+' : ''}${character.initiative}</div>
            </div>
            <div class="character-note-abilities">
                <div><strong>Характеристики:</strong></div>
                <div>Сила: ${character.abilities.str} (${character.modifiers.str >= 0 ? '+' : ''}${character.modifiers.str})</div>
                <div>Ловкость: ${character.abilities.dex} (${character.modifiers.dex >= 0 ? '+' : ''}${character.modifiers.dex})</div>
                <div>Телосложение: ${character.abilities.con} (${character.modifiers.con >= 0 ? '+' : ''}${character.modifiers.con})</div>
                <div>Интеллект: ${character.abilities.int} (${character.modifiers.int >= 0 ? '+' : ''}${character.modifiers.int})</div>
                <div>Мудрость: ${character.abilities.wis} (${character.modifiers.wis >= 0 ? '+' : ''}${character.modifiers.wis})</div>
                <div>Харизма: ${character.abilities.cha} (${character.modifiers.cha >= 0 ? '+' : ''}${character.modifiers.cha})</div>
            </div>
            ${character.equipment && character.equipment.items && character.equipment.items.length > 0 ? `
                <div class="character-note-equipment">
                    <div><strong>Снаряжение:</strong></div>
                    ${character.equipment.items.map(item => `<div>• ${item}</div>`).join('')}
                    ${character.equipment.money ? `<div><strong>Деньги:</strong> ${character.equipment.money}</div>` : ''}
                </div>
            ` : ''}
            ${character.spells && (character.spells.cantrips.length > 0 || character.spells.level_1.length > 0) ? `
                <div class="character-note-spells">
                    <div><strong>Заклинания:</strong></div>
                    ${character.spells.cantrips.length > 0 ? `
                        <div><strong>Заговоры:</strong> ${character.spells.cantrips.join(', ')}</div>
                    ` : ''}
                    ${character.spells.level_1.length > 0 ? `
                        <div><strong>1 уровень:</strong> ${character.spells.level_1.join(', ')}</div>
                    ` : ''}
                </div>
            ` : ''}
            ${character.potions && character.potions.length > 0 ? `
                <div class="character-note-potions">
                    <div><strong>Зелья:</strong></div>
                    ${character.potions.map(potion => `<div>• ${potion.name} - ${potion.effect}</div>`).join('')}
                </div>
            ` : ''}
            ${character.description ? `
                <div class="character-note-description">
                    <div><strong>Описание:</strong> ${character.description}</div>
                </div>
            ` : ''}
            ${character.background_story ? `
                <div class="character-note-story">
                    <div><strong>История:</strong> ${character.background_story}</div>
                </div>
            ` : ''}
        </div>
    `;
