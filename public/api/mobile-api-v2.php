...
try {
    switch ($action) {
        case 'generate_character':
            ...
            // Если API недоступен, используем fallback генерацию
            if (!$result['success'] && isset($result['error']) && strpos($result['error'], 'API недоступен') !== false) {
                $response = generateMobileCharacterFallback($race, $characterClass, $level, $alignment, $gender, $language);
            } else {
...
/**
 * Fallback генерация персонажа для мобильной версии
 */
function generateMobileCharacterFallback($race, $characterClass, $level, $alignment, $gender, $language) {
    try {
        ...
        // Генерируем описание
        $description = generateFallbackDescription($race, $characterClass, $level);
...
function generateCharacterName($race, $namesData) {
    ...
    // Fallback имена
    $fallback_names = ['Арагорн', 'Галадриэль', 'Торин', 'Эовин', 'Боромир', 'Арвен', 'Фродо', 'Сэм'];
    return $fallback_names[array_rand($fallback_names)];
}
...
function getHitDie($characterClass) {
    $hitDice = [
        ...
        'ranger' => 10, 'barbarian' => 12, 'bard' => 8, 'druid' => 8,
        'monk' => 8, 'paladin' => 10, 'sorcerer' => 6, 'warlock' => 8, 'artificer' => 8
    ];
...
function getArmorProficiencies($characterClass) {
    $proficiencies = [
        ...
        'wizard' => [], 'rogue' => ['light'], 'cleric' => ['light', 'medium'],
        'ranger' => ['light', 'medium'], 'barbarian' => ['light', 'medium'],
        'bard' => ['light'], 'druid' => ['light', 'medium'], 'monk' => [],
        'paladin' => ['light', 'medium', 'heavy'], 'sorcerer' => [],
        'warlock' => ['light'], 'artificer' => ['light', 'medium']
    ];
...
function getPrimaryAbility($characterClass) {
    $primary_abilities = [
        'fighter' => 'str', 'wizard' => 'int', 'rogue' => 'dex', 'cleric' => 'wis',
        'ranger' => 'dex', 'barbarian' => 'str', 'bard' => 'cha', 'druid' => 'wis',
        'monk' => 'dex', 'paladin' => 'str', 'sorcerer' => 'cha', 'warlock' => 'cha', 'artificer' => 'int'
    ];
...
function calculateDamage($characterClass, $abilities, $level) {
    ...
    $base_damage = [
        'fighter' => '1d8', 'wizard' => '1d6', 'rogue' => '1d4', 'cleric' => '1d8',
        'ranger' => '1d8', 'barbarian' => '1d12', 'bard' => '1d8', 'druid' => '1d6',
        'monk' => '1d6', 'paladin' => '1d8', 'sorcerer' => '1d6', 'warlock' => '1d4', 'artificer' => '1d8'
    ];
...
function getMainWeapon($characterClass) {
    $weapons = [
        'fighter' => 'Меч', 'wizard' => 'Посох', 'rogue' => 'Кинжал', 'cleric' => 'Булава',
        'ranger' => 'Лук', 'barbarian' => 'Топор', 'bard' => 'Рапира', 'druid' => 'Посох',
        'monk' => 'Кулаки', 'paladin' => 'Меч', 'sorcerer' => 'Посох', 'warlock' => 'Кинжал', 'artificer' => 'Молот'
    ];
...
function getSavingThrows($characterClass, $abilities, $proficiency_bonus) {
    ...
    $class_saving_throws = [
        'fighter' => ['str', 'con'], 'wizard' => ['int', 'wis'], 'rogue' => ['dex', 'int'],
        'cleric' => ['wis', 'cha'], 'ranger' => ['str', 'dex'], 'barbarian' => ['str', 'con'],
        'bard' => ['dex', 'cha'], 'druid' => ['int', 'wis'], 'monk' => ['str', 'dex'],
        'paladin' => ['wis', 'cha'], 'sorcerer' => ['con', 'cha'], 'warlock' => ['wis', 'cha'], 'artificer' => ['con', 'int']
    ];
...
function getClassSpells($characterClass, $level) {
    ...
    // Заклинания 1 уровня
    if ($level >= 1) {
        $level1_spells = ['Свет', 'Магическая стрела', 'Лечение ран', 'Щит', 'Обнаружение магии', 'Компрессионная волна'];
...
function translateRace($race) {
    $translations = [
        ...
        'dragonborn' => 'Драконорождённый', 'gnome' => 'Гном', 'half-elf' => 'Полуэльф',
...
function translateClass($class) {
    $translations = [
        'fighter' => 'Воин', 'wizard' => 'Волшебник', 'rogue' => 'Плут', 'cleric' => 'Жрец',
        'ranger' => 'Следопыт', 'barbarian' => 'Варвар', 'bard' => 'Бард', 'druid' => 'Друид',
        'monk' => 'Монах', 'paladin' => 'Паладин', 'sorcerer' => 'Чародей', 'warlock' => 'Колдун', 'artificer' => 'Артифисер'
    ];
...
function translateAlignment($alignment) {
    $translations = [
        ...
        'lawful-neutral' => 'Законно-нейтральный', 'neutral' => 'Нейтральный', 'chaotic-neutral' => 'Хаотично-нейтральный',
...
function translateRaceLanguage($race) {
    $languages = [
        'human' => 'Общий', 'elf' => 'Эльфийский', 'dwarf' => 'Дварфский', 'halfling' => 'Полуросличий',
        'dragonborn' => 'Драконий', 'gnome' => 'Гномский', 'half-elf' => 'Эльфийский',
        'half-orc' => 'Орчий', 'tiefling' => 'Инфернальный'
    ];
