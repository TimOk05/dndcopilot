...
echo "</div>\n<div class='test-section info'>\n";
echo "<h2>🔄 Тест 5: Проверка обработки ошибок API</h2>\n";

try {
    echo "<h3>Тест с несуществующей расой (должен вернуть ошибку):</h3>\n";
    $error_race = $dnd_service->getRaceData('nonexistent_race');
    
    if ($error_race === null) {
        echo "<p class='success'>✅ API корректно обрабатывает несуществующие расы</p>\n";
    } else {
        echo "<p class='error'>❌ API должен был вернуть null для несуществующей расы</p>\n";
    }
    
    echo "<h3>Тест с несуществующим классом (должен вернуть ошибку):</h3>\n";
    $error_class = $dnd_service->getClassData('nonexistent_class');
    
    if ($error_class === null) {
        echo "<p class='success'>✅ API корректно обрабатывает несуществующие классы</p>\n";
    } else {
        echo "<p class='error'>❌ API должен был вернуть null для несуществующего класса</p>\n";
    }
    
    echo "<p class='success'>✅ Обработка ошибок API работает корректно</p>\n";
    
} catch (Exception $e) {
    echo "<p class='error'>❌ Ошибка обработки ошибок API: " . $e->getMessage() . "</p>\n";
}
...
echo "<li>Кэширование помогает снизить нагрузку на внешние API</li>\n";
        echo "<li>Обработка ошибок API обеспечивает стабильную работу системы</li>\n";
echo "</ul>\n";
echo "</div>\n";
?>
