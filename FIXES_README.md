...
- **Решение**:
  ...
  - Убраны все tooltip для кнопок статистики и админа
  - Исправлено позиционирование tooltip с ограничением ширины
- **Статус**: ✅ Исправлено

### 11. **Проблемы с генерацией имен персонажей**
- **Проблема**: 
  - Мужские имена генерируются для женских персонажей (например, "Тимофей" для женщины)
  - Неправильная логика выбора имен в зависимости от пола
- **Решение**:
  - Исправлена логика выбора имен: сначала имена для конкретного пола, потом унисекс, только в крайнем случае имена другого пола
  - Улучшены fallback имена (D&D тематика: Торин, Арвен, Галадриэль и т.д.)
...
- `test-names-fix.php` - Тест исправлений имен персонажей
