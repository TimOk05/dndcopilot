...
class HttpClient {
    ...
    /**
     * Получение статистики HTTP запросов
     */
    public function getStats() {
        return [
            'cache_enabled' => $this->cacheEnabled,
            'default_timeout' => $this->defaultOptions['timeout'],
            'user_agent' => $this->defaultOptions['user_agent'],
            'ssl_verify' => $this->defaultOptions['verify_ssl']
        ];
    }
    ...
    public function setUserAgent($userAgent) {
        $this->defaultOptions['user_agent'] = $userAgent;
    }
    ...
    /**
     * Включение/отключение SSL проверки
     */
    public function setSSLVerification($verify) {
        $this->defaultOptions['verify_ssl'] = $verify;
    }
     ...
     */
    public function isUrlAccessible($url, $timeout = 5) {
        try {
            $response = $this->get($url, ['timeout' => $timeout]);
            return $response !== false;
        } catch (Exception $e) {
            return false;
        }
     ...
     */
    public function getHeaders($url, $options = []) {
