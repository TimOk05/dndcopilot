...
class TranslationService {
    ...
    /**
     * Получение перевода термина (универсальный метод)
     */
    public function translate($term, $category = null) {
        if ($category && isset($this->translations[$category][$term])) {
            return $this->translations[$category][$term];
        }
        
        // Поиск по всем категориям
        foreach ($this->translations as $cat => $terms) {
            if (isset($terms[$term])) {
                return $terms[$term];
            }
        }
        
        return $term; // Возвращаем оригинал, если перевод не найден
    }
    
    /**
     * Обратный перевод (с русского на английский)
     */
    public function reverseTranslate($russianTerm, $category = null) {
        if ($category && isset($this->translations[$category])) {
            $englishTerm = array_search($russianTerm, $this->translations[$category]);
        ...
        // Поиск по всем категориям
        foreach ($this->translations as $cat => $terms) {
            $englishTerm = array_search($russianTerm, $terms);
            if ($englishTerm !== false) {
                return $englishTerm;
            }
        }
        
        return $russianTerm; // Возвращаем оригинал, если обратный перевод не найден
    }
    
    /**
     * Получение всех терминов категории
     */
    public function getCategoryTerms($category) {
        return $this->translations[$category] ?? [];
    }
     ...
     */
    public function hasTerm($term, $category = null) {
        if ($category) {
            return isset($this->translations[$category][$term]);
        }
        
        foreach ($this->translations as $terms) {
            if (isset($terms[$term])) {
                return true;
            }
        }
    ...
    public function getTranslationStats() {
        $stats = [
            'total_categories' => count($this->translations),
            'total_terms' => 0,
            'categories' => []
        ];
        
        foreach ($this->translations as $category => $terms) {
            $termCount = count($terms);
            $stats['total_terms'] += $termCount;
            $stats['categories'][$category] = $termCount;
        }
    ...
    /**
     * Поиск терминов по частичному совпадению
     */
    public function searchTerms($query, $category = null) {
        $results = [];
        $query = strtolower($query);
        
        if ($category && isset($this->translations[$category])) {
            foreach ($this->translations[$category] as $english => $russian) {
                if (strpos(strtolower($english), $query) !== false || 
                    strpos(strtolower($russian), $query) !== false) {
                    $results[] = [
                        'english' => $english,
                        'russian' => $russian,
                        'category' => $category
            ...
            foreach ($this->translations as $cat => $terms) {
                foreach ($terms as $english => $russian) {
                    if (strpos(strtolower($english), $query) !== false || 
                        strpos(strtolower($russian), $query) !== false) {
                        $results[] = [
                            'english' => $english,
